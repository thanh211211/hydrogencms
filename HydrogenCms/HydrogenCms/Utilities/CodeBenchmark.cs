...
namespace HydrogenCms.Utilities
{
	public sealed class CodeBenchmark : IDisposable
	{
		...
		{
			Hashtable settings = ConfigurationManager.GetSection("HydrogenCms.Utilities.CodeBenchmark") as Hashtable;
			...
			if (settings != null)
			{
				...
				_logFile = HttpContext.Current.Server.MapPath(settings["logFile"] as string);
			}
		}
